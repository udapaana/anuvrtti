...
| built payload | `static/data/readings.json` — generated, committed **for now** |
| quiz cells | `static/data/quiz-cells.json` — generated by vidyut, committed **for now** |

Everything under `static/data/` is build output and is being moved out of git —
see [`ARCHITECTURE.md`](ARCHITECTURE.md). Nothing about how you author changes;
what changes is that your commits stop carrying megabytes you did not write.
...
> **Changing.** The intent here is right — derived data is where a mis-annotation
> becomes visible, since a wrong विभक्ति is invisible in the YAML and obvious in
> the grid. But the diff is 575 lines of JSON plus two whole-file rewrites of
> minified payloads, which means it is never actually read, and two sessions
> authoring in parallel conflict on files neither of them edited. The inspection
> is moving to a generated change report — what cells this reading filled, what
> roots it introduced, what it left untyped — and the JSON is leaving git. See
