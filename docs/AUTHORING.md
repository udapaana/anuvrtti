...
| built payload | `static/data/readings.json` — generated, **gitignored** |
| quiz cells | `static/data/quiz-cells.json` — generated by vidyut, **gitignored** |

Build output is no longer committed: `npm run build:data` rebuilds all of it
from `content/` in about fourteen seconds, and CI does the same before every
deploy. Your commits carry what you wrote and nothing else. See
...
`npm run build` regenerates everything derived, `build:quiz` included — it used
to be left out, which is why `quiz-cells.json` and `usage.json` had to be
committed and rebuilt by hand. They are gitignored now and cost 14s to rebuild.

The instruction that stood here — rebuild deliberately, inspect the diff, commit
it with the reading — was right about the *reason*: a wrong विभक्ति is invisible
in the YAML and obvious in the grid, so derived data is where a mis-annotation
surfaces. It was wrong about the *medium*. The diff was 575 lines of JSON plus
two whole-file rewrites of minified payloads, so it was never actually read, and
two sessions authoring at once conflicted on files neither had edited. The
inspection moves to a generated change report — what cells this reading filled,
what roots it introduced, what it left untyped. Until that exists, `bun run
check` is the gate.
