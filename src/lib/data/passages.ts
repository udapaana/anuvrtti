 ...
 */
export async function loadPassageSource(sourceId: string): Promise<PassageFile | null> {
 ...
 */
export async function getPassage(passageId: string): Promise<ReadingPassage | null> {
 ...
 */
export async function getPassagesBySource(sourceId: string): Promise<ReadingPassage[]> {
  const file = await loadPassageSource(sourceId);
 ...
 */
export async function getAllPassages(sources?: string[]): Promise<ReadingPassage[]> {
...
export async function getPassagesForGrammar(
  ...
  return allPassages.filter(p => {
    ...
    return p.prerequisiteGrammar.every(prereq => completedPaths.includes(prereq));
  });
}
 ...
 * Get source metadata
 */
export async function getSourceMetadata(sourceId: string): Promise<TextSource | null> {
  const file = await loadPassageSource(sourceId);
