...
/** Parse sūtra type from raw type string like "S$वृद्धिसंज्ञा$" or "AD$विभाषाधिकारः$22038" */
function parseType(typeStr: string): { type: SutraType; name: string } {
  if (!typeStr) return { type: "other", name: "" };

  // Handle multiple types separated by ##, take the first one
  const firstType = typeStr.split("##")[0];

  // Match patterns like "S$name$" or "AD$name$id" or "AT$name$"
  const match = firstType.match(/^([A-Z]+)\$([^$]*)\$?/);
  ...
  const typeMap: Record<string, SutraType> = {
    ...
    AD: "adhikara", // अधिकार - governing rule
    AT: "atidesa", // अतिदेश - extension
  };
