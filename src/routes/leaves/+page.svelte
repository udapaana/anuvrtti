<script lang="ts">
  ...
  import { getLeafSutras, type Sutra } from '$lib/data';
  import Sanskrit from '$lib/components/Sanskrit.svelte';

  let sutras: Sutra[] = $state([]);
  let loading = $state(true);
  let groupBy = $state<'adhyaya' | 'type'>('adhyaya');

  onMount(async () => {
    sutras = await getLeafSutras();
    loading = false;
  });
...
<svelte:head>
  <title>Leaf Sutras | Ashtadhyayi Explorer</title>
</svelte:head>

<div>
  <div class="flex items-center justify-between mb-6">
    <div>
      <h1 class="text-2xl font-semibold">Leaf Sutras</h1>
      <p class="text-stone-500 text-sm mt-1">
        Sutras that don't contribute anuvrtti to any later sutra - terminal rules
      </p>
    </div>
