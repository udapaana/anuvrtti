<script lang="ts">
  ...
  // Return-to-previous-page affordance. The layout stashes the path the user
  // came from in sessionStorage when they click `settings`; we read it here.
  let returnTo = $state<string>('/');
  onMount(() => {
    if (typeof sessionStorage === 'undefined') return;
    const stored = sessionStorage.getItem('anuvrtti-settings-return');
    if (stored && stored !== '/settings') returnTo = stored;
  });
