...
function parseMarkdown(content: string): LearningPath {
  ...
  const steps = sections
    .map(parseStep)
    .filter((s): s is LearningStep => s !== null);
...
function generateTypeScript(paths: LearningPath[]): string {
  ...
  paths.forEach((p) => {
    ...
    lines.push(`  estimatedTime: "${p.estimatedTime}",`);
    lines.push(
      `  prerequisites: [${p.prerequisites.map((pr) => `"${pr}"`).join(", ")}],`,
    );
    lines.push("  steps: [");
    ...
    p.steps.forEach((step) => {
      ...
      if (step.keyTerms?.length) {
        lines.push(
          `      keyTerms: [${step.keyTerms.map((t) => `"${escapeString(t)}"`).join(", ")}],`,
        );
      }
      lines.push("    },");
    });
  ...
  // Generate exports
  const varNames = paths.map(
    (p) => p.id.replace(/-./g, (x) => x[1].toUpperCase()) + "Path",
  );
  lines.push(
    `export const learningPaths: LearningPath[] = [${varNames.join(", ")}];`,
  );
  lines.push("");
...
if (args.includes("--to-ts")) {
  ...
  console.log("Usage:");
  console.log(
    "  npx tsx scripts/sync-paths.ts --to-ts   # Markdown -> TypeScript",
  );
  console.log(
    "  npx tsx scripts/sync-paths.ts --to-md   # TypeScript -> Markdown",
  );
}
